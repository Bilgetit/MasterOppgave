...
\begin{proof}
    ...
    Then we consider the loci which define the lines in \ref{eq:li}, denoted $l_i = V(g_i)$(!?). Then all the lines $l_1, \dots l_m$ are included in $V(\Pi_{i=1}^m g_i)$, while $x$ is not. 
...
\begin{proof}
    ...
    Since we can choose $j(x_0,x_1,x_2)$ and $h(x_0,x_1,x_2,x_3)$ freely, we get that $\dim{A_m} = \dim \mathbb{C}[x_0,x_1,x_2]_{d-m} + \dim \mathbb{C}[x_0,x_1,x_2,x_3]_{d-1} = \binom{d-m+2}{2} + \binom{d+2}{3} - 1$. We subtract 1 to account for the fact that we are working in projective space. (!?)
\end{proof}
