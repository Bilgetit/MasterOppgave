...
\end{lemma}
\begin{proof}
    Assume for contradiction that $S = V(g)$ contains a plane $H$. We let $H = V(x_3)$ without loss of generality (refer to a lemma or previous discussion?)(!?). Then $x_3$ divides $g$, and we write $g = x_3 h$ for some $h \in \Bbbk [x_0,x_1,x_2,x_3]_{d-1}$. We then use the Jacobi criterion (15.12 in EO!?) on $x_3 h$, and get that $S$ is singular on the whole of $V(x_3) \cap V(h)$. This is non-empty by Bezout's theorem (\cref{thm:bezout}), giving a contradiction. (Use $d>1$ to see that $\text{deg}(h) = d-1 > 0$)(!?). 
\end{proof}
