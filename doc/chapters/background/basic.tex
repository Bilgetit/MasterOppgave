...
First, a $\emph{linear system}$ on a scheme $X$ is a pair $\mathcal{D} = (\mathcal{L}, V)$, where $\mathcal{L}$ is a line bundle on $X$ and $V \subseteq H^0(X, \mathcal{L})$ a vector space of sections. We have the following theorem:
\begin{theorem}[Bertini's theorem]\label{thm:bertini}
    If $\mathcal{D}$ is a finite dimensional linear system on a variety $X$ in characteristic $0$, then the general member of $\mathcal{D}$ is smooth away from the base locus of $\mathcal{D}$ and the singular locus of $X$.
\end{theorem}
We refer to \cite[Corollary III.10.9]{Ha77} for a proof. 
...
\end{theorem}
We refer to \cite[Corollary 2.4]{EH16} for a proof. 
...
\begin{theorem}[\cite{Vak25} Corollary 12.4.2]
...
\end{theorem}
\begin{theorem}[\cite{Vak25} Corollary 12.2.1]
    If $X$ is an irreducible $k$-variety, then $\dim X = \mathrm{trdeg} K(X)/k$.  
\end{theorem}
\begin{theorem}[Dimension formula for general fibers] \label{thm:fiber}
    Let $k$ be a field and let $f\colon X \to Y$ be a dominant morphism of irreducible varieties over $k$. Let $\eta$ denote the generic point of $Y$ and write $X_\eta = X \times_Y \mathrm{Spec}\,k(Y)$ for the generic fiber. Then
    \[
...
\end{lemma}
This can be found in \cite[Exercise 14.4.21]{EO25}.

\begin{theorem}[Upper semicontinuity of fiber dimension]\label{thm:uppersemicont}
    Let $k$ be a field, and suppose $\pi \colon X \to Y$ is a morphism of finite type $k$-schemes. If $\pi$ is proper, then the dimension of the fiber of $\pi$ over $q \in Y$ is an upper semicontinuous function in $q$ (i.e., on $Y$). That is, for each integer $n \geq 0$, the set
    \[
        \{y \in Y \mid \dim f^{-1}(y) \geq n\}
    \]
    is closed in $Y$.
\end{theorem}
This can be found in \cite[Theorem 12.4.3]{Vak25}.
...
% \end{proof}
\input{chapters/background/fiber_dim}