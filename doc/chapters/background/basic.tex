...
\begin{theorem}[\cite{Vak25} 11.4.1]
    Suppose $\pi \colon X \to Y$ is a (necessarily finite type) morphism of irreducible $k$-varieties, with $\dim X = m$ and $\dim Y = n$. Then there exists a nonempty open subset $U \subseteq Y$ such that for all $q \in U$, the fiber over $q$ has pure dimension $m-n$, or is empty.
\end{theorem}
