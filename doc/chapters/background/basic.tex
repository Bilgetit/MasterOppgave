\section{Notation and important results}
...
\begin{theorem}[Bézout]\label{thm:bezout}
    Let $X, Y \subseteq \mathbb{P}^n$ be two closed subvarieties having complementary dimension, i.e. $\dim(X) + \dim(Y) = n$. If $X$ and $Y$ intersect transversely, then they will intersect in exactly $\deg(X) \cdot \deg(Y)$ points.
\end{theorem}
...
\todo[inline]{Jørgen: Is X and Y intersect in finitely many points, then the number of intersection points is at most deg(x)deg(Y).}
