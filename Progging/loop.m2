...
time0 = currentTime();
d = 2;
while currentTime() - time0 < 5*60 do(  -- wait for 5 minutes
    maxNoPoints = ceiling((d+3)*(d+2)*(d+1)/(6*d));
    for noPoints from maxNoPoints - 1 to maxNoPoints do ( -- maxNoPoints to maxNoPoints + 1 for m-chains
        ...
        -- Ensure f is not the zero polynomial unless Iz has no nonzero elements of degree d
